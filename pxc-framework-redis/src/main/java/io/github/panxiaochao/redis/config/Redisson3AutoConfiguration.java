...
public class Redisson3AutoConfiguration {
  ...
  public RedissonAutoConfigurationCustomizer redissonAutoConfigurationCustomizers(
    ...
    final ObjectMapper objectMapper = objectMapperObjectProvider.getIfAvailable(
    ...
    return config -> {
      ...
      Method sentinelServersMethod = ReflectionUtils.findMethod(Config.class,
          "getSentinelServersConfig");
      Method clusterServersMethod = ReflectionUtils.findMethod(Config.class,
          "getClusterServersConfig");
      // 自定义前缀
      KeyPrefixNameMapper keyPrefixNameMapper = new KeyPrefixNameMapper(
          redisson3Properties.getKeyPrefix());
      // 使用单机模式, 使用自定义前缀

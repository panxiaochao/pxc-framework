...
public class Ipv6Address {
    ...
    private Ipv6Address(byte[] mIp) {
        this.mIp = mIp;
    }

    public static Ipv6Address fromBytesV6(byte[] b) {
        if (b.length == INADDR16SZ) {
            return new Ipv6Address(b);
        } else if (b.length < INADDR16SZ) {
            byte[] bs = new byte[INADDR16SZ];
            System.arraycopy(b, 0, bs, 0, b.length);
            return new Ipv6Address(bs);
        } else {
            throw new IllegalArgumentException("非法IP地址");
        }
    }

    public static Ipv6Address fromBytesV6LE(byte[] b) {
        byte[] be = bsWap(b);
    ...
    public int compareTo(Ipv6Address anIp) {
        // TODO Arrays.compareUnsigned Jdk 11

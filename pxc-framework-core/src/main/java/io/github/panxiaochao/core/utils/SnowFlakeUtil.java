 ...
 * <p>
 * datacenterId（分布式）（服务ID 1，2，3.....） 每个服务中写死 machineId（用于集群） 机器ID 读取机器的环境变量MACHINEID 部署时每台服务器ID不一样
 *
 * @author Lypxc
 * @since 2022-12-22
 */
public class SnowFlakeUtil implements Serializable {
  ...
  public synchronized long nextId() {
    ...
    return ((timestamp - startTimestamp) << timestampLeftShift) | (dataCenterId
        << dataCenterIdShift)
  ...
  protected long tilNextMillis(long lastTimestamp) {
    ...
    if (timestamp < lastTimestamp) {
      ...
      throw new IllegalStateException(
          String.format("Clock moved backwards. Refusing to generate id for %dms",
              lastTimestamp - timestamp));
    }
    return timestamp;
  }

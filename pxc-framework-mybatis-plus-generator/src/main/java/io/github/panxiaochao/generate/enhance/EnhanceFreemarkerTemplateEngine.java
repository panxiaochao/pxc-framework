...
public class EnhanceFreemarkerTemplateEngine extends FreemarkerTemplateEngine {
    ...
    private void outputApplication(ConfigBuilder config) {
        ...
        if (StringUtils.hasText(config.getPackageConfig().getModuleName())) {
            parentPackage = parentPackage.replaceAll(StringPoolUtil.DOT + config.getPackageConfig().getModuleName(), StringPoolUtil.EMPTY);
            parentPath = parentPath.replace(config.getPackageConfig().getModuleName(), StringPoolUtil.EMPTY);
        }
    ...
    private void outputPom(ConfigBuilder config) {
        String outputDir = config.getGlobalConfig().getOutputDir();
        outputDir = outputDir.replace(SRC_MAIN_JAVA, StringPoolUtil.EMPTY);
        Map<String, Object> objectMap = new HashMap<>(3);
